...
class ApiTestCase(TestCase):
    def setUp(self):
        ...
        self.file_content = "This is some test file data for an " "uploaded file."
        ...
        uploaded_file = SimpleUploadedFile(self.fn, str.encode(self.file_content))
        tu = TemporaryUpload(
    ...
    def test_store_upload_unset_file_store_path(self):
        ...
        with self.assertRaisesMessage(
            ...
            "A required setting is missing in " "your application configuration.",
        ):
    ...
    def test_store_upload_invalid_id(self):
        with self.assertRaisesMessage(
            ValueError, "The provided upload ID " "is of an invalid format."
        ):
            store_upload("hsdfiuysh78sdhiu", "/test_storage/test_file.txt")
    ...
    def test_store_upload_invalid_id_correct_format(self):
        with self.assertRaisesMessage(
            ValueError, "Record for the specified " "upload_id doesn't exist"
        ):
            store_upload("hsdfiuysh78sdhiuf73gds", "/test_storage/test.txt")

    def test_store_upload_path_none(self):
        with self.assertRaisesMessage(
            ValueError, "No destination file " "path provided."
        ):
    ...
    def test_store_upload_path_with_filename(self):
        ...
        LOG.debug(
            "About to check that file path <%s> and stored path <%s> "
        ...
        self.assertFalse(
            os.path.exists(os.path.join(upload_tmp_base, self.upload_id, self.file_id))
        )
    ...
    def test_store_multiple_uploads_to_same_dir(self):
        ...
        # Check second file
        self.assertTrue(
            os.path.exists(file_full_path2) and os.path.isfile(file_full_path2)
        )
        self.assertFalse(
            os.path.exists(os.path.join(upload_tmp_base, self.upload_id2, self.file_id))
        )
    ...
    def tearDown(self):
        ...
        if os.path.exists(test_target_file) and os.path.isfile(test_target_file):
            LOG.debug("Removing test target file: <%s>" % test_target_file)
            os.remove(test_target_file)
        if os.path.exists(test_target_file2) and os.path.isfile(test_target_file2):
        ...
        if os.path.exists(test_target_file_fn) and os.path.isfile(test_target_file_fn):
            LOG.debug("Removing test_target_file_fn:<%s>" % test_target_file_fn)
            os.remove(test_target_file_fn)
        if os.path.exists(test_target_dir_fn) and os.path.isfile(test_target_dir_fn):
            LOG.debug("Removing test_target_dir_fn:<%s>" % test_target_dir_fn)
            os.remove(test_target_dir_fn)
        # Remove directory
        if os.path.exists(test_target_dir) and os.path.isdir(test_target_dir):
