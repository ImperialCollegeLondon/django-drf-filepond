...
``DJANGO_DRF_FILEPOND_ALLOW_EXTERNAL_UPLOAD_DIR`` (*default*: ``False``):

	When files are initially uploaded from a client and handled by
    django-drf-filepond, they are stored to a temporary storage location on
    disk (set by `DJANGO_DRF_FILEPOND_UPLOAD_TMP`, or to a default location
    if this configuration option is not set). By default, django-drf-filepond
    does not allow this temporary storage directory to be outside of the main
    project directory (i.e. it must be a sub-directory of the main project
    directory). If you wish to use a directory elsewhere on the filesystem
    as the base location for temporary storage of file uploads, and you
    understand any potential risks that this may present, you can set this
    value to `True` to enable setting of `DJANGO_DRF_FILEPOND_UPLOAD_TMP`
    to a location outside of the project directory.

``DJANGO_DRF_FILEPOND_MAX_FETCH_BYTES`` (*default*: ``100MB``):

	Filepond's `fetch` functionality, enables a user to drop a URL of a remote
    file onto the filepond client and have the server retrieve this file. The
    file is stored as a temporary upload as if it had been uploaded from the
    client. To prevent the server being directed to download a very large file
    that could result in exhausting the available memory or disk space on the
    system, a 100MB (1024^2 bytes * 100) limit is set for uploads requested
    using `fetch`. You can change this limit by setting the
    `DJANGO_DRF_FILEPOND_MAX_FETCH_BYTES` configuration option to an integer
    value specifying the maximum number of bytes that can be retrieved using
    using fetch. (added in 0.5.3).


``DJANGO_DRF_FILEPOND_MAX_CHUNK_DATA_SIZE`` (*default*: ``5000000 bytes``):

    When using chunked uploads, the incoming chunk data is stored in memory.
    To avoid attempts to send very large file chunks that could, for example,
    exhaust available system memory, this parameter allows tuning of the
    maximum allowed chunk size that can be received and stored in memory.
    This defaults to 5000000 bytes since this is the default `chunkSize`
    set in the filepond client. See https://pqina.nl/filepond/docs/api/instance/properties/
    for further information. If you are handling very large uploads and
    increase the `chunkSize` value on the filepond client, you will need to
    ensure that you also increase this value in your application configuration.
    However, note that Django has its own `DATA_UPLOAD_MAX_MEMORY_SIZE`
    parameter that defaults to 2.5MB (see
    https://docs.djangoproject.com/en/6.0/ref/settings/#data-upload-max-memory-size).
    You may also need to adjust this parameter on your Django configuration
    if you're seeing a `RequestDataTooBig` exception generated by Django.
